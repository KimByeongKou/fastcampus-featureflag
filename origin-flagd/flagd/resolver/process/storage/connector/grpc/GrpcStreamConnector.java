...
public class GrpcStreamConnector implements Connector {
    ...
    static void observeEventStream(final BlockingQueue<StreamPayload> writeTo,
        ...
        final BlockingQueue<dev.openfeature.contrib.providers.flagd.resolver.process.storage.connector.grpc.GrpcResponseModel> streamReceiver = new LinkedBlockingQueue<>(QUEUE_SIZE);
        int retryDelay = INIT_BACK_OFF;

        while (!shutdown.get()) {
            serviceStub.syncFlags(request, new dev.openfeature.contrib.providers.flagd.resolver.process.storage.connector.grpc.GrpcStreamHandler(streamReceiver));

            while (!shutdown.get()) {
                final dev.openfeature.contrib.providers.flagd.resolver.process.storage.connector.grpc.GrpcResponseModel response = streamReceiver.take();
