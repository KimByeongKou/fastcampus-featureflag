...
public class FlagStore implements Storage {
    ...
    private final Map<String, dev.openfeature.contrib.providers.flagd.resolver.process.model.FeatureFlag> flags = new HashMap<>();

    private final dev.openfeature.contrib.providers.flagd.resolver.process.storage.connector.Connector connector;

    public FlagStore(final dev.openfeature.contrib.providers.flagd.resolver.process.storage.connector.Connector connector) {
     ...
     */
    public dev.openfeature.contrib.providers.flagd.resolver.process.model.FeatureFlag getFlag(final String key) {
        readLock.lock();
        try {
    ...
    private void streamerListener(final Connector connector) throws InterruptedException {
        final BlockingQueue<dev.openfeature.contrib.providers.flagd.resolver.process.storage.connector.StreamPayload> streamPayloads = connector.getStream();
