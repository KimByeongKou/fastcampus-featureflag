...
public class InProcessResolver implements Resolver {
    ...
    private final dev.openfeature.contrib.providers.flagd.resolver.process.targeting.Operator operator;
    ...
    private <T> ProviderEvaluation<T> resolve(Class<T> type, String key,
            EvaluationContext ctx) {
        final dev.openfeature.contrib.providers.flagd.resolver.process.model.FeatureFlag flag = flagStore.getFlag(key);
        ...
        if (dev.openfeature.contrib.providers.flagd.resolver.process.model.FeatureFlag.EMPTY_TARGETING_STRING.equals(flag.getTargeting())) {
