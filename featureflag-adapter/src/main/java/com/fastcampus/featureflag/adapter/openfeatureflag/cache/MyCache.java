...
public class MyCache {
    private Map<String, ProviderEvaluation<? extends Object>> lruCache;
     ...
     */
    public MyCache(final String forType, int maxCacheSize, FeatureflagCounter counter) {
        ...
        if (DISABLED.getValue().equals(forType)) {
            ...
            this.myCache = cacheManager.createCache("myCache",
    ...
    public ProviderEvaluation<? extends Object> get(String key) {
        ...
        if (result != null) {
            ...
            if (cacheType.equals("LRU")){
                ...
                this.myCache.forEach(entry -> count.getAndIncrement());

                if(count.get() == cacheSize) {
                    counter.incrementEvictionCount();
                }
            }

            this.counter.incrementCacheMiss();
        }
